...
class MyEnv(fym.BaseEnv):
    ...
    def get_Lambda(self, t):
        ...
        # Lambda = np.ones((11, 1))
        _Lambda = np.vstack([0.7, 0.8, 0.9, 1, 1, 1, np.ones((5, 1))])

        if t < (t1 := 3):
            Lambda = np.ones((11, 1))
        elif t < (t2 := 7):
            Lf0 = np.ones((11, 1))
            Lf = np.vstack([_Lambda[0], np.ones((10, 1))])
            Lambda = (Lf0 - 1 * Lf) * ((t2 - t) / (t2 - t1)) ** 2 + Lf
        elif t < (t3 := 12):
            Lf0 = np.vstack([_Lambda[0], np.ones((10, 1))])
            Lf = np.vstack([_Lambda[0], _Lambda[1], np.ones((9, 1))])
            Lambda = (Lf0 - 1 * Lf) * ((t3 - t) / (t3 - t2)) ** 2 + Lf
        elif t < (t4 := 15):
            Lf0 = np.vstack([_Lambda[0], _Lambda[1], np.ones((9, 1))])
            Lf = np.vstack([_Lambda[0], _Lambda[1], _Lambda[2], np.ones((8, 1))])
...
def plot():
    ...
    ylabels = np.array(("$d_1$", r"$d_2$", r"$d_3$", r"$d_4$"))
    for i, _ylabel in enumerate(ylabels):
        ax = axs[i]
        ax.plot(data["t"], data["dhat"].squeeze(-1)[:, i], "k-", label="Estimate")
        ax.plot(data["t"], data["d"].squeeze(-1)[:, i], "r--", label="Actual")
        ax.grid()
        plt.setp(ax, ylabel=_ylabel)
        if i == 0:
            ax.legend(loc="upper right")
    plt.gcf().supxlabel("Time, sec")
    plt.gcf().supylabel("Disturbance Estimation")
