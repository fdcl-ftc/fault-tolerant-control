...
class Main:
    ...
    def generate_raw(self, N, seed=1):
        ...
        phitheta = np.random.uniform(*np.deg2rad((-10, 10)), size=(N, 2, 1))
        psi = np.random.uniform(*np.deg2rad((-10, 10)), size=(N, 1, 1))
        angle = np.hstack((phitheta, psi))
        pq = np.random.uniform(*np.deg2rad((-10, 10)), size=(N, 2, 1))
        r = np.random.uniform(*np.deg2rad((-10, 10)), size=(N, 1, 1))
        omega = np.hstack((pq, r))
